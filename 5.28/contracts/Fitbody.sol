...
contract Fitbody is Ownable {
    ...
    //创建健身人的私有构造函数
    function _createMember(uint _backMuscle, uint _chestMuscle,uint _armMuscle, uint _legMuscle, uint _vitality, string memory _name, string memory _sno, uint256 _amount) private {
        //require(msg.value >= 0.001 ether);

        uint256 id = members.push(member( _backMuscle,  _chestMuscle,_armMuscle, _legMuscle,  _vitality, _name, _sno , _amount, now, 1, 0)) - 1;
    ...
    function RestBody (uint256 id) external {
        ...
        //否则表示休息完成 增加体力值
            ...
            //计算经过的小时
            uint restHour = (now - myMember.LaststartRestTime)/36;
            //每小时增加一个体力
